...
#' 校验同花顺jqka数据中的两种主要类型rtype/period以及数据转换方向drc
#' @param x 待检验的项
#' @param jktype 财报相关的主要类型，包括rtype/period/drc
#' @return 返回F or T，标识校验是否通过。
...
#' 建立数据库本地数据库通道
#' @param dtype 数据库类型，目前支持postgres 与 mysql
...
get.jkdic <- function(rtype = 'debt', src = NULL){
  ...
  if(is.null(src) || src == 0){
    ...
    if(status_code(r) == 200){
      #提取文本内容
      ct = content(r, 'text', encoding = 'UTF-8')
    ...
    #读取字典中的数据SQL
    rdics = try(dbGetQuery(fcon, sql),
                silent = T) %>% as.data.table()
