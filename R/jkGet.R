...
jkCorl <- function(dt, rtype = 'debt', corlby = 'subject'){
  ...
  if(corlby == 'subject'){
    ...
    abn = res[is.na(name)]$subject %>% paste(., collapse = '|')
...
#jkadr为文件地址(包含文件名)，需关联mysql-jkdic table
jkRead <- function(jkdir = '.', rtype = 'debt'){
  ...
  #读取文件
  res = try(read_excel(adr), silent = T)
...
#' 计算各财报中需要增加的额外数据，grate, xrate用于main报表中进行简易的估值计算
#' @param dt data.table格式数据
...
jkCalc <- function(dt, rtype = 'debt', grate = 1, xrate = 0.08){
  ...
  if(rtype == 'debt'){
    ...
    if(grate == 1){
      ...
      #以近3年复合增长率 计算3年后的收入
      #再以折半的增长率，计算10年后的收入
      r[, c('bs1') := fv(r = ror1, n = 3, pv = epsAvg) %>% abs()]
      r[, c('bs2') := fv(r = ror1/2, n = 7, pv = bs1) %>% abs()]
      ...
      # 关键还是预测的增长率情况！！！
      r[, c('bs1') := fv(r = grate, n = 5, pv = epsAvg) %>% abs()] #五年后的收益值
      r[, c('bs2') := fv(r = grate/2, n = 5, pv = bs1) %>% abs()] #十年后的收益值
